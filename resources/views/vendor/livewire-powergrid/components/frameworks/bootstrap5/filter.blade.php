...
<div x-data="{ open: @entangle("showFilters").live }" class="mt-2 md:mt-0">
    <div x-show="open" x-cloak x-transition:enter="transform duration-100" x-transition:enter-start="opacity-0 scale-90"
        x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transform duration-100"
        x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-90" class="py-3"
        wire:key="{{ uniqid() }}">
        @php
            $customConfig = [];
        @endphp
                ...
                @foreach ($filters as $filter)
                    @if (str(data_get($filter, "className"))->contains("FilterMultiSelect"))
                        <div class="{{ data_get($filter, "baseClass") }}">
                            <x-livewire-powergrid::inputs.select :inline="false" :tableName="$tableName" :filter="$filter"
                                :theme="$theme->filterMultiSelect" :initialValues="data_get(data_get($filters, "multi_select"), data_get($filter, "field" ), [])" />
                        </div>
                    @endif
